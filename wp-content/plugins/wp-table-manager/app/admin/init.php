...
if (!function_exists('wptm_update_tables')) {
    ...
    function wptm_update_tables()
    {
        ...
        if (version_compare($version, '2.7.0', '>=') && $version !== '2.7.2' && !$updated_tables) {
        ...
        if (version_compare($version, '2.7.0', '<') && $version !== '2.7.2') {
        ...
        $tables_convert = get_option('wptm_tables_convert', null);
        if (version_compare($version, '2.7.0', '>=') && $version !== '2.7.2' && $tables_convert !== null && (int)$tables_convert === -1) {
...
if (!function_exists('wptm_update_version')) {
    ...
    {
        $version = get_option('wptm_version');
        if (version_compare($version, '2.3.0', '<') && $version !== '2.7.2') {
        ...
        //update default theme when update plugin
        if (version_compare($version, '2.7.1', '<') && $version !== '2.7.2') {
            if (function_exists('wptm_update_styles')) {
                global $wpdb;
...
function wptm_init()
{
    ...
    if (($page === 'wptm' || $page === 'wptm-folderTree') && $dbtable !== 'dbtable') {
        ...
        if (isset($tid) && $tid !== 0) {
            ...
            wp_enqueue_script('handsontable', plugins_url('assets/js/handsontable.full.js', __FILE__), array(), WPTM_VERSION);
    ...
    wp_localize_script('wptm-main', 'wptmText', array(
        ...
        'CHANGE_INVALID_CELL_DATA' => __('Invalid cell data', 'wptm'),
        ...
        'CHART_NOT_EXIST' => __('No charts are created from the table, thanks!', 'wptm'),
        ...
        'error_move_category' => esc_html__('<p>Has error ordering the category, please <a onClick="window.location.reload();">reload</a> the page!</p>', 'wptm'),
