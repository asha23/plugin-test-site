...
jQuery(document).ready(function ($) {
    ...
    if (typeof wptm_convert_data !== 'undefined') {
        if (wptm_convert_data.id > 0) {
            convert_data_table(wptm_convert_data.id);
        } else {
            $.ajax({
    ...
    $('#create_new').unbind('click').on('click', function () {
        ...
        if (Wptm.roles.capabilities.wptm_access_database_table) {
        ...
        $('#create_new_menu').find('.create_category:not(.no_click)').unbind().on('click', function () {
            ...
            var popup = {
                ...
                'showAction': function () {
                    ...
                    for (id in list_cat) {
                        if (typeof list_cat[id] !== 'undefined') {
                            this.find('#jform_parent_cat').append('<option value="' + list_cat[id][1] + '">' + list_cat[id][0] + '</option>');
                        }
                    }
    ...
    function update_changer(content) {
        ...
        if ($('#inserttable').length > 0) {//change insert button status
            ...
            $('#inserttable').data('type', 'table').attr('data-type', 'table').text(insert_table).addClass("no_click");
            if (typeof Wptm.table !== 'undefined') {
                $('#inserttable').removeClass("no_click");
            }
        }

        // if (Wptm.showDbTables) {
        //     $folder_path.contents().remove();
        //
        //     var html = '<div data-id="0"><span>TABLES</span></div>', i;
        //
        //     html += '<div data-id="' + 0 + '"><span>' + wptmText.table_from_database + '</span></div>';
        //
        //     //print folder_path
        //     $folder_path.append(html);
        //     $wptm_categories_list.find('.db_cat').addClass('active');
        // } else {
        //     $wptm_categories_list.find('.db_cat').removeClass('active');
        // }
        ...
        $categories.find('.ui-droppable').contextmenu(function (e) {
            ...
            right_click.call(e, function () {
                ...
                $right_mouse_menu.find('.rename_table_menu').unbind('click').on('click', () => {
                    re_name_cat(this);
                });
                    ...
                    if (Wptm.notMoved === 1 || pk == cat_target) {
                        ...
                        $.ajax({
                            ...
                            if (result.response === true) {
                                ...
                                status_noti(0, wptmText.error_move_category);
                            }
                        });
        ...
        $list_tables.find('.dd-item').unbind('click').click(function (e) {
            ...
            if ($('#inserttable').length > 0 && !$(e.target).hasClass('hasChart')) {
                $list_tables.find('.tbody.active').trigger('click');
                $('#inserttable').removeClass("no_click").data('type', 'table').attr('data-type', 'table').text(insert_table);
            }
        ...
        $list_tables.find("tbody").sortable({
            ...
            stop: function (event, ui) {
                ...
                if (Wptm.notMoved == 0 && Wptm.id_cat_target > 0 && Wptm.id_cat_target != Wptm.cat_active) {
                    $(ui.item).hide();
        ...
        $list_tables.find('.dd-item').contextmenu(function (e) {
            ...
            right_click.call(e, function () {
                ...
                if (!wptm_permissions.can_edit_tables && (!$(this.currentTarget).hasClass('hasRole') || !wptm_permissions.can_edit_own_tables)) {
                    $right_mouse_menu.find('.edit_table_menu').addClass('wptm_hiden');
                    $right_mouse_menu.find('.rename_table_menu').addClass('wptm_right_menu_top');
                } else {
    ...
    function re_name_cat (that) {
        ...
        var popup = {
            ...
            'showAction': function () {
                this.find('.wptm_done').addClass('wptm_blu');
                this.find('.wptm_cancel').addClass('wptm_grey');
                this.find('#jform_re_name-lbl').text(wptmText.rename_category);
    ...
    function update_chart () {
        ...
        if (parseInt(chart_active) > 0) {
            list_chart.find('.tbody[data-id="' + chart_active + '"]').trigger('click');
            chart_active = 0;
        }
    ...
    */
    ...
    function setCategory ($idTable, content) {
        if (parseInt($idTable) > 0) {
            $.ajax({
                ...
                if ($categories.find('.dd-item[data-id-category="' + id_cat + '"]')) {
    ...
    function smallDisplay () {
        if (parseInt($('#mybootstrap.wptm-tables').width()) < 680 && $wptm_categories_list.find('.wptm_show_cat').length < 1) {
            ...
            $wptm_show_cat.toggle(
                function () {
                    $(this).val('Hide').addClass('show');
                    $wptm_categories_list.animate({width: '100%', opacity: '1'}, "slow");
                },
                function () {
                    $(this).val('Show').removeClass('show');
    ...
    function check_role (action, data) {
        var role = Wptm.roles.capabilities;
        ...
        switch (action) {
            case 'showDbTable':
                ...
                break;
            case 'hasRole':
                if (role.wptm_edit_tables || (Wptm.idUser == data.author && role.wptm_edit_own_tables)) {
                    return true;
                }
                return false;
                break;
